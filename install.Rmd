...
First restart your Rstudio session (e.g., using Session > Restart R) and use the code below to make sure all required packages packages are up to date

```r
radiant.update::radiant.update()
```

If all packages are up to date, restart your Rstudio session again and use the code below to install the development version of the Radiant packages

```r
packages <- c("radiant.data", "radiant.design", "radiant.basics", "radiant.model", "radiant.multivariate", "radiant")
sapply(packages, install.packages, repos = "https://radiant-rstats.github.io/minicran/dev/")
```

If you need to revert to the current (stable) release of Radiant use code below 
