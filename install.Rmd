...
> **Note:** R 3.4 or higher is now required to use the latest version of Radiant. This follows after R dropped support for R 3.3 (on Mac)
...
The recommend approach to start Radiant is `Start radiant (window)` from the `Addins` menu. This will start Radiant in an Rstudio window. Closing this window will also (cleanly) stop Radiant. If you are using an Rstudio project, any file opened or saved will default to the project directory. This also makes it possible to use `relative` paths to load data and images in `Report > Rmd`.
